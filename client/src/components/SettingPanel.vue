    ...
    <div class="setting-panel-box" v-if="this.$store.state.set.selectedSettingTool === `audio`">
      ...
      <div style="display:flex;justify-content: space-around;">
        <button class="btn btn-danger btn-tool btn-record" @click="recordRestart()">{{
          this.btnType=="Pause" ? "一時停止" : "プレイ" }}</button>
        <button class="btn btn-primary btn-tool btn-record" @click="clear()">クリア</button>
        <button class="btn btn-success btn-tool btn-record" @click="recordAdd()">追加</button>
      </div>
    </div>
