  ...
  <div class="text-bar media-line"
    ...
    <div v-if="this.$store.state.set.selectedSettingTool === `text`" v-for="(text, id) in this.$store.state.upload.texts"
      ...
      <div v-if="this.active[id] === `active`" class="text" @click="selectItem(id)" @contextmenu="onContextMenu($event)"
        @mousedown="resizeSelected($event, 2, id)">
        {{
    ...
    <div id="context-menu-text">
      <div class="item" @click="del_text()">Delete</div>
    </div>
    <div v-if="this.$store.state.set.selectedSettingTool !== `text`" v-for="(text, id) in this.$store.state.upload.texts"
...
export default {
  ...
  mounted() {
    ...
    window.addEventListener("click", (e) => {
      if (document.getElementById("context-menu-text")) {
        const contextMenu = document.getElementById("context-menu-text");
